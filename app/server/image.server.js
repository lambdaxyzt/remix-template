...
export const mainImageReadStream = async (url) => {
    ...
    console.log(isContentThere, "is mane image there ?")
    if (!isContentThere) {
        await fetch(url).then(res => {
            // pipe the result stream into a file on disc
            return pipeline(
                res.body,
                cacache.put.stream(cachePathMainImage, url).on('integrity', d => console.log(`integrity digest is ${d}`))
            )
        })
        console.log("finished pipe line")
    }
...
export const generatedImageReadstream = async (mainImageReadstream, url, width, quality) => {
    ...
    console.log(urlKey)
    console.log(await isThereImage(urlKey), "isCondddddtentThere")
    console.log(isContentThere, "isContentThere")
    console.log(await cacache.get.info(cachePathImage, urlKey))
